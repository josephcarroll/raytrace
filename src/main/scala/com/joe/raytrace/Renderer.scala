...
object Renderer {
  ...
  def render(pixelWidth: Int, pixelHeight: Int, antialiasing: Int)(scene: Scene): Array[Vector] = time("rendering") {
    ...
    time("tracing") {
      for (y <- (0 until renderHeight).par; x <- 0 until renderWidth) {
